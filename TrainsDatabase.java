...
public class TrainsDatabase {
	...
	public static void editTrain(Train train , Database database) throws SQLException {
		String update ="UPDATE `trains` SET `Capacity`='"+train.getCapacity()+"' ,`Description`= '"+train.getDescription()+"' "
				+ " WHERE `ID` = '"+train.getID()+"' ;";
		database.getStatement().execute(update);
	}
	
	public void DeleteTrain(String id, Database database) throws SQLException {
	...
	public static  Train getTrainByDescription(String description , Database database) throws SQLException {
		...
		ResultSet rs= database.getStatement().executeQuery(select);
		 if (rs.next()) {
		        ...
		        System.out.println("No train found with Description: " + description);
		    }

...
public class TripsDatabase {
    ...
    public static void AddTrip(Trip t, Database database) throws SQLException {
        ...
        //String insert = "INSERT INTO trips(`ID`, `Start`, `Destination`, `DepartureTime`, `ArrTime`, `Date`, `Price`, `Driver`, `Train`) VALUES 
              //  + "('" + t.getId() + "','" + t.getStart() + "','" + t.getDestination() + "','" + t.getDepartureTime() 
              //  + "','" + t.getArrivalTime() + "','" + t.getDate() + "', '" + t.getPrice() + "','" + t.getDriver() + "','" + t.getTrain() + "')";
    	String insert = "INSERT INTO trips (`ID`, `Start`, `Destination`, `DepartureTime`, `ArrTime`, `Date`,`BookedSeats`,`Price`, `Driver`, `Train`) VALUES ('"
    		    + t.getID() + "', '"
    		    + t.getStart() + "', '"
    		    + t.getDestination() + "', '"
    		    + t.getDepartureTime() + "', '"
        ...
        // Create Passengers table for the trip
    ...
    public static Trip getTrip(String id, Database database) throws SQLException {
        ...
        if (rs.next()) {
            ...
            throw new SQLException("No trip found with ID: " + id);
        }
    }

    
    public static void EditTrip(Trip t ,Database database) throws SQLException {
    	String update = "UPDATE `trips` SET `Start` = '" + t.getStart() + "', "
    		    + "`Destination` = '" + t.getDestination() + "', "
    		    + "`DepartureTime` = '" + t.getDepartureTime() + "', "
    		    + "`ArrTime` = '" + t.getArrivalTime() + "', "
    		    + "`Date` = '" + t.getDate() + "', "
    		    ...
    		    + "WHERE `ID` = " + t.getID() + ";";
 /*   String update = "  UPDATE `trips` SET  `Start`='"+t.getStart()+"',`Destination`='"+t.getDestination()+"',`DepartureTime`='"+t.getDepartureTime()+"',`ArrTime`='"+t.getArrivalTime()+"',`Date`='"+t.getDate()+"',`BookedSeats`='"+t.getBookSeats()+"',`Price`='"+t.getPrice()+"',`Driver`='"+t.getDriver().getID()+"',`Train`='"+t.getTrain().getID()+"' WHERE 'ID'  = "+t.getID()+" 
  * ;";*/
    ...
    public static void DeleteTrip(String id, Database database) throws SQLException {
        ...
        // Correctly format the DROP TABLE query
    ...
    public static String[][] getPassengers(String id, Database database) throws SQLException {
        String select = "SELECT Passenger, Tickets FROM `Trip" + id + "Passengers`;";
        ResultSet rs = database.getStatement().executeQuery(select);
        ...
        while (rs.next()) {
            ids.add(String.valueOf(rs.getInt("Passenger")));
            nums.add(String.valueOf(rs.getInt("Tickets")));
        }

        // Now create a 2D array where:
        // array[0] contains Passenger IDs
        // array[1] contains corresponding Ticket numbers
        String[][] array = new String[2][ids.size()];

        for (int i = 0; i < ids.size(); i++) {
            array[0][i] = ids.get(i);  // Passenger ID
            array[1][i] = nums.get(i);  // Ticket numbers
        }

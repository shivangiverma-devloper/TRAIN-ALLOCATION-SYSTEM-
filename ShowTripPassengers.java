...
public class ShowTripPassengers {
	...
	public ShowTripPassengers (JFrame parent , Database database) throws SQLException {
		...
		JFrame frame =new JFrame("Show Trip Pasengers");
  ...
  private void refreshTable(Database database) throws SQLException {
	...
	//  gridLayout.setRows(data[0].length+1);
	  ...
	  row1.add(JLabel("ID",50));
	  row1.add(JLabel("Name", 150));
	  row1.add(JLabel("Tel" ,150));
	  row1.add(JLabel("Email", 150));
	  row1.add(JLabel("Tickets",80));
	  table.add(row1);
	  ...
	  for (int i = 0; i < data[0].length; i++) {
		    Passenger p = PassengersDatabase.getPassenger(data[0][i], database);
		    JPanel row = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 10));
		    row.setBorder(BorderFactory.createEmptyBorder(25, 0, 0, 0));
		    row.setBackground(i % 2 == 1 ? Color.decode("#EEEEEE") : Color.decode("#e5e5e5"));
		    row.add(JLabel(String.valueOf(p.getID()), 50));
		    row.add(JLabel(p.getName(), 150));
		    row.add(JLabel(p.getTel(), 150));
		    row.add(JLabel(p.getEmail(), 150));

		    // Debugging: Check if ticket data is available
		    if (data.length > 1 && data[1].length > i) {
		        System.out.println("Ticket data for passenger " + p.getID() + ": " + data[1][i]);
		        row.add(JLabel(data[1][i], 80));
		    } else {
		        System.out.println("No ticket data available for passenger " + p.getID());
		        row.add(JLabel("N/A", 80));  // Add a placeholder if no data is available
		    }
		    table.add(row);
		}
